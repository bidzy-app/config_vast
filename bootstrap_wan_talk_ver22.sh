...
print_listeners(){
  log "Sockets listening (18188/8188/18288, 3000):"
  (ss -ltnp 2>/dev/null || netstat -tulpn 2>/dev/null) | egrep ':18188|:8188|:18288|:3000' || true
}
...
start_wrapper_shim(){
  # Лёгкий HTTP-shim на 127.0.0.1:18288, который принимает /runsync и прокидывает в ComfyUI /prompt
  if (ss -ltnp 2>/dev/null || netstat -tulpn 2>/dev/null) | grep -qE ':18288\b'; then
    log "Wrapper shim: port 18288 already in use, skipping"
    return 0
  fi
...
def main():
    ...
    print("shim listening on 127.0.0.1:18288 ->", COMFY, flush=True)
...
maybe_start_pyworker(){
  ...
  if curl -fsSL --retry 5 "$URL" -o /tmp/start_udp.sh; then
    ...
    export BACKEND WORKER_PORT MODEL_LOG USE_SSL UNSECURED MODEL_SERVER_URL
...
main(){
  ...
  # Важно: shim поднимаем перед PyWorker — он ждёт /runsync на 18288
  start_wrapper_shim
